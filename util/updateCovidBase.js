...
const updateCovidData = async (ctx) => {
	...
	if (data) {
		...
		// });
		db.getConnection(function (err, connection) {
			connection.query("TRUNCATE covid_countries", (err) => console.log(err));
			connection.query(
				"INSERT INTO covid_countries (country, country_code, slug, new_confirmed, total_confirmed, new_deaths, total_deaths, new_recovered, total_recovered) VALUES ?",
				[values],
				(err) => console.log(err)
			);
		});
